...
The k-nearest neighbours, the random forest and the support vector machine models returned similar predictive accuracies although slightly higher for the random forest (72%) and the k-nearest neighbours (70%) algorithms. Using maturity classes or ionomics groups, the predictive accuracies for all the models on test set are also similar. 
...
```{r}
test_df$ypred = predicted_rf_grp # adds predictions to test set
...
#ggsave("images/cultivarAcc.tiff", width=5, height=8)
```
