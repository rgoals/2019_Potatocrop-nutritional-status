...
ggsave("images/cluster_plot.png", width=10, height=6, dpi = 300)
```
...
g2 <- ggplot(data=lda_loading) +
  ...
  theme(axis.text=element_text(size = 12),
        text = element_text(family = "Arial", face = "bold", size = 12))
plot_grid(g1, g2)
ggsave("images/discriminant_plots.png", width=10, height=6, dpi = 300)
```

The first discriminant axis (`DS1`) is formed mainly by `Mg`, `K` and `N`. The second discriminant axis (`DS2`) is driven mainly by `Fv`, `K` and weakly by `Mg`, `Ca` and `P`. I append discriminant scores to leaves composition data frame.
...
```{r, effect-plot, fig.height = 8, fig.width = 5, out.width = "100%", fig.align = "center", warning = FALSE, message = FALSE, fig.cap = "Effects of clr nutrients on tubers yield."}
samples_tidy %>%
  filter(!is.na(clr)) %>% 
  ...
  theme(axis.text=element_text(size = 12),
        text = element_text(family = "Arial", face = "bold", size = 12)) +
  theme_bw()
ggsave("images/bayes-mixed-clr.png", width=5, height=8, dpi = 300)
```
