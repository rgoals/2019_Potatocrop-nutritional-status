...
## Clustering potato cultivars with ionome
...
```{r, cluster-plot, out.width = "100%", fig.align = "center", warning = FALSE, message = FALSE, fig.cap = "Cluster plot of poato cultivars based on centered log-ratio N P K Mg Ca transformed compositions of diagnostic leaves."}
...
I plot the biplot as result of PCA in separate plots for easy reading. 

```{r, pca-grid-plot, fig.height = 5, fig.width = 10, out.width = "100%", fig.align = "center", warning = FALSE, message = FALSE, fig.cap = "Grid plot of potato ionome principle component analysis."}
...
The first principle axis or component (`PC1`) is formed mainly by `Mg` and `K`. The second axis (`PC2`) is driven mainly by `P` and `Ca`.

## Do clrs affect potato tuber yield?

We will calculate the importance of the clrs effect on tuber yield using `varImp()` method for object producced by `train` in machine learning methods, in the next chapter (Chapter \@ref(Chapter-Modeling)).  

I select useful columns in a new table named `dfml.csv`. I conserved only `complete cases`.
...
So, the Machine learning data table contains `r nrow(dfml)` samples. I check the data frame to see cultivars abundance. Cultivar `Goldrush` overcomes the others.
